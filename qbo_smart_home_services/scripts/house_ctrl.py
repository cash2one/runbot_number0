...
LOG_ON = False
TEMP_DB_PATH = '/home/runji/.ros/house.db'
REAL_DB_PATH = '/home/runji/.ros/house.sqlite'

def init_appliance_tbl(tbl):
    #普通灯
    tbl['0'] = {'0':
                 {'req':{u'打开':0x4f, u'关闭':0x4f},
                 'rsp':{u'打开':100, u'关闭':0}},
    #可调节灯
                '1':
    ...
    #窗帘
    tbl['1'] = {'3':
                 {'req':{u'打开':61, u'打关':62, u'暂停':63},
                 'rsp':{u'开闭':100, u'关闭':0, u'暂停':50}}
               }
    #电视
    tbl['2'] = {'4':
                 {'req':{u'打开':0x00, u'关闭':0x01, u'频道加':15, u'频道减':16, u'音量加':18, u'音量减':17}}
               }


def HAS_DB():
    try:
        stat_info = os.stat(REAL_DB_PATH)
    except OSError:
        return False
    if stat_info.st_size < 1024: #invalid if too small
        return False
    db_ctime = stat_info.st_ctime
    create_date = time.strftime('%Y-%m-%d', time.localtime(db_ctime))
...
def get_house_db():
    ...
    fp.close()
    os.system('rm -f ' + REAL_DB_PATH + ';sqlite3 ' + TEMP_DB_PATH +' .dump | iconv -f gb18030 -t utf-8 | sqlite3 ' + REAL_DB_PATH)
...
def get_app_info(room_name, dev_name):
    if not HAS_DB():
        get_house_db()
    db_conn = sqlite3.connect(REAL_DB_PATH)
    c = db_conn.cursor()
